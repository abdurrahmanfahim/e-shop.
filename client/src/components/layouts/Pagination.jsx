...
const Pagination = ({ totalItems, itemsPerPage, currentPage, onPageChange }) => {
  ...
  if (totalPages <= maxPagesToShow) {
    ...
    for (let i = 0; i < uniquePages.length; i++) {
      ...
      if (i < uniquePages.length - 1 && uniquePages[i + 1] - uniquePages[i] > 1) pages.push("...");
    }
  }
  ...
  return (
    <div className="w-full max-w-[522px] mx-auto flex gap-2 sm:gap-[18px] items-center justify-center p-4 font-poppins font-semibold text-base sm:text-xl leading-[30px] text-center text-black">
      ...
      {pages.map((number, index) => (
        <div className="w-full flex justify-center items-center" key={typeof number === "number" ? number : `dots-${index}`}>
          {number === "..." ? (
            <span className="px-10p select-none">...</span>
          ) : (
            <button
              onClick={() => handlePageChange(number)}
              className={`rounded-[5px] py-1.5 px-3 sm:py-[9px] sm:px-[18px] cursor-pointer ${number === currentPage ? "bg-green text-white" : "hover:bg-gray-100"}`}
            >
              {number}
            </button>
          )}
